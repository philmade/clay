...
| **ops_loop** | Operations — runs systems, monitors output, gathers data, reports | When something is built and needs to be operated |
...
2. **Build loop finishes** → You read **{{.HandoffDir}}/MANUAL.md** → Compose detailed report for user
3. **If it needs operating** → Set operational tasks → Transfer to **ops_loop**
4. **Ops loop finishes** → You read **{{.HandoffDir}}/FEEDBACK.md** → Compose detailed report for user
...
The build and ops loops communicate through standardized files in {{.HandoffDir}}/:
